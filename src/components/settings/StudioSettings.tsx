...
export default function StudioSettings({ userId, onNavigate, onSignOut }: StudioSettingsProps) {
  ...
  return <div className="settings-page">
    ...
    <div className="settings-layout">
      <nav className="settings-tabs">{tabs.map(item => { const Icon = item.icon; return <button key={item.id} className={`settings-tab ${tab === item.id ? 'active' : ''}`} onClick={() => setTab(item.id)}><Icon size={16} /> {item.label}</button> })}</nav>
      <div className="settings-content">
        {tab === 'theme' && <div className="settings-section"><h3>Theme & Display</h3><p className="settings-desc">Choose how HkTube should look on this device.</p><div className="settings-choice-grid">{(['dark', 'light', 'system'] as ThemeChoice[]).map(choice => <button key={choice} className={`settings-choice ${theme === choice ? 'active' : ''}`} onClick={() => setTheme(choice)}><strong>{choice === 'dark' ? 'Dark Mode' : choice === 'light' ? 'Light Mode' : 'System Default'}</strong><small>{choice === 'system' ? 'Follow your device preference' : `Use ${choice} colours`}</small></button>)}</div></div>}
        ...
        {tab === 'account' && <div className="settings-section"><h3>Account</h3><p className="settings-desc">Manage your HkTube account and creator access.</p><button className="btn-secondary btn-sm btn-danger settings-logout" onClick={logOut}><LogOut size={14} /> Log Out</button></div>}
