...
export async function rankPublicVideos(input: { shorts?: boolean; limit?: number; query?: string; userId?: string | null }): Promise<RankedVideo[]> {
  ...
  let followed = new Set<string>(); let watched = new Set<string>(); let saved = new Set<string>(); let feedback = new Map<string, string>(); let blocked = new Set<string>();
  if (userId) {
    ...
    followed = new Set((subs.data ?? []).map(x => String(x.channel_id))); watched = new Set((history.data ?? []).map(x => String(x.video_id ?? x.short_id)).filter(Boolean)); saved = new Set((saves.data ?? []).map(x => String(x.video_id)).filter(Boolean)); feedback = new Map((fb.data ?? []).map(x => [String(x.content_id), String(x.feedback_type)])); blocked = new Set((blocks.data ?? []).map(x => String(x.blocked_id)));
  }
  ...
  const queryTokens = tokens(input.query ?? ""); const watchedCandidates = candidates.filter(v => watched.has(v.id)).slice(0, 20); const watchedTokens = watchedCandidates.flatMap(v => tokens(`${v.title} ${v.description ?? ""} ${v.tags.join(" ")}`));
  const ranked = candidates.map((v, index) => {
    const contentTokens = tokens(`${v.title} ${v.description ?? ""} ${v.tags.join(" ")}`); const interest = queryTokens.length ? overlap(queryTokens, contentTokens) : overlap(watchedTokens, contentTokens); const follow = followed.has(v.channelId) ? 1 : 0; const watchQuality = Math.min(1, (Number(v.viewCount) > 0 ? Math.log10(Number(v.viewCount) + 1) / 7 : 0.05) + (Number(v.durationSeconds) > 0 ? 0.08 : 0)); const completion = input.shorts ? Math.min(1, Number(v.likes_count || 0) / Math.max(10, Number(v.viewCount || 0))) : Math.min(1, Number(v.likes_count || 0) / Math.max(20, Number(v.viewCount || 0))); const engagement = Math.min(1, (Number(v.likes_count || 0) * 4 + Number(v.viewCount || 0)) / 10000); const saveValue = saved.has(v.id) ? 1 : 0; const novelty = watched.has(v.id) ? 0 : 1; const queryRelevance = queryTokens.length ? overlap(queryTokens, contentTokens) : 0;
    const base = input.shorts ? 0.25 * completion + 0.18 * (novelty * 0.8 + interest * 0.2) + 0.15 * engagement + 0.10 * saveValue + 0.10 * interest + 0.08 * freshness(v.publishedAt) + 0.07 * follow + 0.07 * novelty : 0.24 * interest + 0.18 * watchQuality + 0.14 * completion + 0.10 * freshness(v.publishedAt) + 0.09 * engagement + 0.08 * follow + 0.06 * queryRelevance + 0.05 * saveValue + 0.04 * novelty + 0.02;
    const penalty = feedback.get(v.id) === "not_interested" || feedback.get(v.id) === "hide_creator" ? 2 : feedback.get(v.id) === "less_like_this" ? 0.7 : 0; const ownerBlocked = ownerByChannel.get(v.channelId) ? blocked.has(ownerByChannel.get(v.channelId)!) : false; const duplicatePenalty = rankedCreatorCountPlaceholder(v.channelId, candidates, index) >= 3 ? 0.45 : 0; const score = ownerBlocked ? -10 : base - penalty - duplicatePenalty - index * 0.0001;
  ...
  for (const item of ranked) { if (output.length >= limit) break; const creatorCount = creatorCounts.get(item.channelId) ?? 0; if (creatorCount >= 3) continue; const topic = item.tags[0] || "general"; const topicCount = topicCounts.get(topic) ?? 0; if (topicCount >= Math.ceil(limit * 0.35)) continue; output.push(item); creatorCounts.set(item.channelId, creatorCount + 1); topicCounts.set(topic, topicCount + 1); }
  return output;
}
function rankedCreatorCountPlaceholder(channelId: string, candidates: SupabaseVideo[], index: number) { let count = 0; for (let i = 0; i < index; i++) if (candidates[i].channelId === channelId) count++; return count; }
