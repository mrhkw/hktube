...
export async function rankPublicVideos(input: { shorts?: boolean; limit?: number; query?: string; userId?: string | null }): Promise<RankedVideo[]> {
  ...
  const ranked = candidates.map((v, index) => {
    ...
    const engagement = Math.min(1, (Number(v.likes_count || 0) * 4 + Number(v.viewCount || 0) + (signal?.shares || 0) * 20 + (signal?.comments || 0) * 8) / 12000);
    ...
    const popularityScore = popularity(Number(v.viewCount || 0), Number(v.likes_count || 0));
