...
async function readDuration(file: File) {
  return new Promise<number>(resolve => {
    ...
    const done = () => {
      const value = Number.isFinite(video.duration) ? Math.max(0, Math.floor(video.duration)) : 0;
...
async function readDimensions(file: File) {
  return new Promise<{ width: number; height: number } | null>(resolve => {
    ...
    video.preload = "metadata";
    const done = () => {
      const result = video.videoWidth > 0 && video.videoHeight > 0 ? { width: video.videoWidth, height: video.videoHeight } : null;
      URL.revokeObjectURL(url);
      resolve(result);
    };
    video.onloadedmetadata = done;
...
export async function listMySupabaseVideos(userId: string) {
  const { data, error } = await supabase.from("videos").select(VIDEO_SELECT).eq("creator_id", userId).order("created_at", { ascending: false }).limit(100);
  if (error) throw new Error(error.message);
  return (data ?? []).map(mapVideo);
}

export async function listPublicSupabaseVideos(limit = 20) {
  const { data, error } = await supabase.from("videos").select(VIDEO_SELECT).eq("visibility", "public").eq("status", "published").eq("moderation_status", "approved").order("published_at", { ascending: false }).limit(limit);
  if (error) throw new Error(error.message);
  return (data ?? []).map(mapVideo);
}

export async function listPublicSupabaseShorts(limit = 40) {
  const { data, error } = await supabase.from("videos").select(VIDEO_SELECT).eq("visibility", "public").eq("status", "published").eq("moderation_status", "approved").contains("tags", ["shorts"]).order("published_at", { ascending: false }).limit(limit);
  if (error) throw new Error(error.message);
  return (data ?? []).map(mapVideo);
}

export async function createSupabaseVideo(input: { channelId: string; title: string; description: string; file: File; thumbnail?: File | null; isShort?: boolean; durationSeconds?: number; category?: string | null; language?: string | null; onProgress?: (value: number) => void }) {
  ...
  if (!dimensions) throw new Error("HkTube could not read the video stream. Re-export it as a standard MP4/H.264 file and try again.");
  if (input.isShort) {
    const ratio = dimensions.width / dimensions.height;
    if (ratio > 1.05) throw new Error("Clips must be vertical (9:16). Please select a portrait video.");
  }
  ...
  const duration = Math.max(0, Math.floor(Number(input.durationSeconds ?? await readDuration(input.file))));
  input.onProgress?.(10);
  ...
  try {
    if (input.thumbnail) {
      ...
      const { error } = await supabase.storage.from("thumbnails").upload(thumbnailPath, input.thumbnail, { contentType: input.thumbnail.type || "image/jpeg", upsert: false, cacheControl: "31536000" });
      if (error) throw new Error(error.message);
    }
    input.onProgress?.(82);
    ...
    // Never leave a storage object behind when database/thumbnail creation fails.
    await supabase.storage.from("videos").remove([videoPath]).catch(() => undefined);
    if (thumbnailPath) await supabase.storage.from("thumbnails").remove([thumbnailPath]).catch(() => undefined);
    throw error;
  }
}
