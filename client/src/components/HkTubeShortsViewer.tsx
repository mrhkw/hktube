...
export function HkTubeShortsViewer({ items, mode, onModeChange }:{items:Item[];mode:"for-you"|"following";onModeChange:(m:"for-you"|"following")=>void}){
  const { user }=useAuth();
  const [muted,setMuted]=useState(true); const [paused,setPaused]=useState<Record<string,boolean>>({});
  ...
  const [ready,setReady]=useState<Record<string,boolean>>({}); const [mediaError,setMediaError]=useState<Record<string,string>>({});
  ...
  async function like(id:string){if(!user)return startLogin();try{const r=await toggleVideoLike(id);setLiked(s=>({...s,[id]:r.liked}));setCounts(s=>({...s,[id]:Number(r.count)}));void recordDiscoveryEvent({eventType:r.liked?"like":"dislike",objectType:"short",objectId:id}).catch(()=>undefined)}catch(e){toast.error(e instanceof Error?e.message:"Unable to like this Short.")}}
  async function save(id:string){if(!user)return startLogin();try{const r=await toggleVideoSave(id);setSaved(s=>({...s,[id]:r}));void recordDiscoveryEvent({eventType:r?"save":"dislike",objectType:"short",objectId:id}).catch(()=>undefined);toast.success(r?"Saved to Library":"Removed from Library")}catch(e){toast.error(e instanceof Error?e.message:"Unable to save this Short.")}}
  ...
  return <div className="fixed inset-0 z-[80] overflow-hidden bg-black text-white">
    ...
    <div className="h-full snap-y snap-mandatory overflow-y-auto overscroll-y-contain [scrollbar-width:none]">
      {items.map((item,index)=>{const video=media("videos",item.video_path);const thumb=media("thumbnails",item.thumbnail_path);const channel=item.channel_id?channels[item.channel_id]:undefined;const hasReady=ready[item.id];const failed=mediaError[item.id];return <section key={item.id} data-short-id={item.id} className="relative flex h-[100dvh] snap-start items-center justify-center overflow-hidden bg-black">
        ...
        {paused[item.id]&&<button onClick={()=>togglePlay(item.id)} className="absolute left-1/2 top-1/2 z-40 grid size-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-black/50 backdrop-blur" aria-label="Play"><Play className="ml-1 size-8 fill-current"/></button>}
      </section>})}
    </div>
