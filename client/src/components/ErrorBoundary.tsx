...
class ErrorBoundary extends Component<Props, State> {
  ...
  static getDerivedStateFromError(error: Error): State { return { hasError: true, errorMessage: error?.message || "Unknown UI error" }; }
  componentDidCatch(error: Error, info: { componentStack: string | null }) {
    console.error("[HkTube] Unhandled UI error", error, info);
  }
  private retry = () => { try { sessionStorage.removeItem("hktube-error-recovery"); } catch {} window.location.reload(); };
    ...
    if (this.state.hasError) return (
      <main className="flex min-h-screen items-center justify-center bg-background p-6">
        <section role="alert" className="flex w-full max-w-md flex-col items-center rounded-2xl border border-border bg-card p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-muted-foreground">The page hit a client-side error. Reloading the app clears stale UI state and retries the current route.</p>
