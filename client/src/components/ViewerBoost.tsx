...
export function ViewerBoost() {
  ...
  return <section className="mx-auto mt-8 w-full max-w-[1480px] px-3 sm:px-6 lg:px-10" aria-label="Viewer discovery">
    <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
      ...
      <Link href="/trending" className="group flex min-h-24 items-center gap-3 rounded-3xl border border-violet-300/15 bg-violet-500/[.08] p-5 transition hover:-translate-y-0.5 hover:bg-violet-500/[.13]"><Sparkles className="size-5 text-violet-200" aria-hidden="true" /><span><span className="block text-sm font-black text-white">Today's discovery</span><span className="mt-1 block text-xs text-slate-400">Open Trending for fresh picks.</span></span><PlayCircle className="ml-auto size-5 text-violet-200 transition group-hover:translate-x-0.5" aria-hidden="true" /></Link>
    </div>
    {items.length > 0 && <div className="mt-6"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black text-white">Continue watching</h2><Link href="/history" className="text-xs font-bold text-violet-200 hover:text-white">View history</Link></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{items.map(video => <Link key={video.id} href={`/watch/${video.id}`} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[.03] transition hover:-translate-y-0.5 hover:border-violet-300/20"><div className="aspect-video bg-white/[.04]">{video.thumbnailUrl ? <img src={video.thumbnailUrl} alt="" className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.02]" loading="lazy" /> : <div className="grid h-full place-items-center"><PlayCircle className="size-8 text-slate-500" aria-hidden="true" /></div>}</div><div className="p-3"><p className="line-clamp-2 text-sm font-bold text-white">{video.title}</p><p className="mt-1 text-xs text-slate-500">Resume watching</p></div></Link>)}</div></div>}
  </section>;
}
