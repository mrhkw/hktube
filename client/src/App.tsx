...
const Home = lazy(() => import("@/pages/SupabaseHome"));
...
const SearchResults = lazy(() => import("@/pages/SupabaseSearch"));
const Upload = lazy(() => import("@/pages/Upload"));
const VideoCollection = lazy(() => import("@/pages/SupabaseCollections").then(m => ({ default: m.SupabaseCollections })));
...
function DeferredUtilities() {
  ...
  useEffect(() => { let cancelled = false; const timer = window.setTimeout(() => { void Promise.all([import("@/components/ui/sonner").then(module => { if (!cancelled) setToaster(() => module.Toaster); }), import("@/components/InstallAppPrompt").then(module => { if (!cancelled) setInstallPrompt(() => module.InstallAppPrompt); })]); }, 900); return () => { cancelled = true; window.clearTimeout(timer); }; }, []);
  return <>{Toaster ? <Toaster /> : null}{InstallPrompt ? <InstallPrompt /> : null}</>;
}
function RouteFallback() { return <div className="grid min-h-[45vh] place-items-center"><div className="rounded-full border border-zinc-200 bg-white/90 px-4 py-2 text-sm font-semibold text-zinc-600 shadow-sm">Loading HkTube…</div></div>; }
function Router() { return <Suspense fallback={<RouteFallback />}><Switch>
