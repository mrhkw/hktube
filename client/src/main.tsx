...
try { const savedTheme = window.localStorage.getItem("hktube-theme") || "violet"; document.documentElement.dataset.hktubeTheme = THEME_IDS.has(savedTheme) ? savedTheme : "violet"; } catch { document.documentElement.dataset.hktubeTheme = "violet"; }
...
const trpcClient = trpc.createClient({ links: [httpBatchLink({ url: "/api/trpc", transformer: superjson, async headers() { const { data } = await supabase.auth.getSession(); return data.session?.access_token ? { Authorization: `Bearer ${data.session.access_token}` } : {}; }, fetch: authFetch })] });

function SafeEnhancements() {
  const [LanguageRuntime, setLanguageRuntime] = useState<ComponentType | null>(null); const [AccountBootstrap, setAccountBootstrap] = useState<ComponentType | null>(null); const [ThemeRuntime, setThemeRuntime] = useState<ComponentType | null>(null);
