...
export function SupabaseCollections({ kind }: { kind: "shorts" | "trending" }) {
  ...
  if (kind === "shorts") {
    ...
    return <ClipsView items={items.map(v => ({ id: v.id, title: v.title, description: v.description, video_path: v.videoUrl, thumbnail_path: v.thumbnailUrl, views: v.viewCount, likes_count: v.likesCount, channel_id: v.channelId, tags: v.tags }))} mode={mode} onModeChange={setMode} />;
  }
