...
function Notifications({ items, onRead }: { items: Array<{ id: number; title: string; body: string | null; href: string | null; readAt: Date | null; createdAt: Date }>; onRead: () => void }) {
  ...
  const markAllRead = trpc.notifications.markAllRead.useMutation({ onSuccess: onRead, onError: error => toast.error(error.message) });
  const [sort, setSort] = useState<"newest" | "unread">("newest");
  const visible = [...items].sort((a, b) => sort === "unread" ? Number(Boolean(a.readAt)) - Number(Boolean(b.readAt)) : new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  return <div className="mx-auto max-w-2xl space-y-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="flex gap-2"><Button type="button" size="sm" variant={sort === "newest" ? "default" : "outline"} onClick={() => setSort("newest")}>Newest</Button><Button type="button" size="sm" variant={sort === "unread" ? "default" : "outline"} onClick={() => setSort("unread")}>Unread first</Button></div><Button type="button" size="sm" variant="ghost" disabled={markAllRead.isPending || !items.some(item => !item.readAt)} onClick={() => markAllRead.mutate()}>Mark all read</Button></div>{visible.map(item => <div key={item.id} className={`flex items-start justify-between gap-4 rounded-2xl border p-4 ${item.readAt ? "border-white/8 bg-white/[.02]" : "border-fuchsia-400/20 bg-fuchsia-400/[.06]"}`}><div><p className="font-semibold text-white">{item.href ? <Link href={item.href} className="hover:text-fuchsia-200">{item.title}</Link> : item.title}</p>{item.body && <p className="mt-1 text-sm text-slate-400">{item.body}</p>}<p className="mt-2 text-xs text-slate-600">{formatDate(item.createdAt)}</p></div>{!item.readAt && <Button variant="outline" size="sm" onClick={() => markRead.mutate({ id: item.id })}>Mark read</Button>}</div>)}</div>;
}
