...
export default function SupabaseHome() {
  const [userId, setUserId] = useState<number | string | undefined>(undefined);
  ...
  useEffect(() => {
    let cancelled = false;
    void import("@/_core/hooks/useAuth").then(({ useAuth }) => { void useAuth; }).catch(() => undefined);
    try {
      const raw = window.localStorage.getItem("hktube-user-id");
      if (raw && /^\d+$/.test(raw)) setUserId(Number(raw));
    } catch { /* anonymous feed remains available */ }
  ...
  async function feedback(videoId: string, type: "not_interested" | "hide_creator" | "hide_topic" | "more_like_this" | "less_like_this") {
    try {
      ...
      if (type === "not_interested" || type === "hide_creator" || type === "hide_topic" || type === "less_like_this") {
        setVideos(items => items.filter(v => v.id !== videoId)); setShorts(items => items.filter(v => v.id !== videoId));
  ...
  const topics = useMemo(() => { const map = new Map<string, number>(); videos.forEach(v => v.tags.filter(tag => tag !== "shorts").forEach(tag => map.set(tag, (map.get(tag) || 0) + 1))); return [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10); }, [videos]);
  ...
  return <HkTubeShell>
    <main className="mx-auto w-full max-w-[1480px] px-4 pb-16 sm:px-8 lg:px-10">
      <section className="mb-8 overflow-hidden rounded-[28px] border border-white/10 bg-[#101522] p-6 sm:p-10"><div className="flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-300">HkTube discovery</p><h1 className="mt-2 text-4xl font-black text-white sm:text-6xl">Watch what matters.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">Personalized discovery built from real HkTube activity, fresh uploads, followed creators and explicit recommendation feedback.</p></div><div className="flex flex-wrap gap-3"><Link href="/search" className="inline-flex items-center gap-2 rounded-full border border-white/15 px-5 py-2.5 text-sm font-bold text-white"><Search className="size-4" />Search</Link><Link href="/shorts" className="rounded-full bg-white px-5 py-2.5 text-sm font-bold text-black">Open Shorts</Link></div></div><div className="mt-7 flex flex-wrap gap-2">{topics.map(([topic, count]) => <Link key={topic} href={`/search?q=${encodeURIComponent(topic)}`} className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5 text-xs font-semibold text-slate-300 hover:border-violet-300/30 hover:text-white">#{topic} · {count}</Link>)}</div></section>
