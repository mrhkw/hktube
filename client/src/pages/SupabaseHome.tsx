...
export default function SupabaseHome() {
  ...
  const recommended = useMemo(() => uniqueById(feedPool).slice(0, 12), [feedPool]);
  ...
  const feedTitle = personalizedFeed ? "Recommended for you" : "Latest on HkTube";
  ...
  const risingNow = useMemo(() => uniqueById([...videos].sort((a, b) => Number(b.viewCount || 0) - Number(a.viewCount || 0))).filter(v => !usedAfterRecommended.has(v.id)).slice(0, 8), [videos, usedAfterRecommended]);

  return <HkTubeShell><main className="mx-auto w-full max-w-[1500px] px-4 pb-16 sm:px-7 lg:px-9">
    <section className="mb-8 border-b border-white/7 pb-7 pt-5 sm:pt-2"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-[11px] font-bold uppercase tracking-[.2em] text-violet-300">Your HkTube</p><h1 className="mt-2 text-3xl font-black tracking-tight text-white sm:text-4xl">Find something worth watching.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Discovery, fresh uploads and creators you follow — without putting creator tools in the viewing experience.</p></div><div className="mb-5 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-6"><Link href="/" className="rounded-2xl border border-violet-300/20 bg-violet-500/10 px-3 py-3 text-left"><span className="block text-xs font-bold text-violet-200">Home</span><span className="mt-1 block text-[11px] text-slate-500">{personalizedFeed ? "For you" : "Latest"}</span></Link><Link href="/shorts" className="rounded-2xl border border-white/8 bg-white/[.025] px-3 py-3 text-left"><span className="block text-xs font-bold text-white">Clips</span><span className="mt-1 block text-[11px] text-slate-500">Quick watch</span></Link><Link href="/trending" className="rounded-2xl border border-white/8 bg-white/[.025] px-3 py-3 text-left"><span className="block text-xs font-bold text-white">Trending</span><span className="mt-1 block text-[11px] text-slate-500">Rising now</span></Link><Link href="/subscriptions" className="rounded-2xl border border-white/8 bg-white/[.025] px-3 py-3 text-left"><span className="block text-xs font-bold text-white">Following</span><span className="mt-1 block text-[11px] text-slate-500">Your creators</span></Link><Link href="/library" className="rounded-2xl border border-white/8 bg-white/[.025] px-3 py-3 text-left"><span className="block text-xs font-bold text-white">Library</span><span className="mt-1 block text-[11px] text-slate-500">Saved videos</span></Link><Link href="/studio" className="rounded-2xl border border-white/8 bg-white/[.025] px-3 py-3 text-left"><span className="block text-xs font-bold text-white">Studio</span><span className="mt-1 block text-[11px] text-slate-500">Create & manage</span></Link></div><div className="flex flex-wrap gap-2"><Link href="/search" className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[.035] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-white/[.07]"><Search className="size-4" />Search</Link><Link href="/explore" className="rounded-full bg-violet-500 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-violet-400">Explore</Link></div></div>{topics.length > 0 && <div className="mt-5 flex flex-wrap gap-2">{topics.map(([topic, count]) => <Link key={topic} href={`/search?q=${encodeURIComponent(topic)}`} className="rounded-full border border-white/8 bg-white/[.025] px-3 py-1.5 text-xs font-semibold text-slate-400 transition hover:border-violet-300/30 hover:text-white">#{topic}<span className="ml-1.5 text-slate-600">{count}</span></Link>)}</div>}</section>
    {loading ? <div className="grid min-h-[42vh] place-items-center"><div className="flex items-center gap-3 text-sm text-slate-400"><Loader2 className="size-5 animate-spin text-violet-300" />Loading your feed…</div></div> : error ? <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-white/[.03] p-8 text-center"><p className="font-semibold text-white">We couldn’t load your feed.</p><p className="mt-2 text-sm text-slate-500">{error}</p><Button onClick={() => void load()} className="mt-5 bg-violet-500 hover:bg-violet-400"><RefreshCw className="mr-2 size-4" />Retry</Button></div> : videos.length ? <div className="space-y-11"><Section title="Your HkTube" eyebrow="Choose what to watch"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Link href="/shorts" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 transition hover:border-violet-300/30 hover:bg-violet-500/[.06]"><p className="font-bold text-white">Shorts</p><p className="mt-1 text-xs text-slate-500">Vertical swipe videos</p></Link><Link href="/posts" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 transition hover:border-violet-300/30 hover:bg-violet-500/[.06]"><p className="font-bold text-white">Posts</p><p className="mt-1 text-xs text-slate-500">Community updates</p></Link><Link href="/stories" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 transition hover:border-violet-300/30 hover:bg-violet-500/[.06]"><p className="font-bold text-white">Stories</p><p className="mt-1 text-xs text-slate-500">24-hour updates</p></Link><Link href="/subscriptions" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 transition hover:border-violet-300/30 hover:bg-violet-500/[.06]"><p className="font-bold text-white">Following</p><p className="mt-1 text-xs text-slate-500">Creators you follow</p></Link></div></Section>
      ...
      {recommended.length > 0 && <Section title="Long Videos for You" eyebrow={personalizedFeed ? "Personalized discovery" : "Latest uploads"}><div className="grid gap-x-5 gap-y-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{recommended.map(v => <SupabaseVideoCard key={`recommended-${v.id}`} video={card(v, type => void feedback(v.id, type))} />)}</div></Section>}
    ...
    <div className="mt-9 flex flex-wrap items-center gap-3 text-xs text-slate-600"><span className="inline-flex items-center gap-2"><Sparkles className="size-3.5" />{personalizedFeed ? "Your feed learns from watches, saves, follows, shares and explicit feedback." : "Personalization is paused. Home is using fresh public uploads."}</span><span className="rounded-full border border-white/8 px-2.5 py-1 text-slate-500">{freshnessLabel}</span></div>
  </main></HkTubeShell>;
}
