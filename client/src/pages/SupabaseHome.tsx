...
export default function SupabaseHome() {
  ...
  return <HkTubeShell><main className="mx-auto w-full max-w-[1500px] px-4 pb-16 sm:px-7 lg:px-9">
    <section className="mb-8 border-b border-white/7 pb-7 pt-5 sm:pt-2">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2"><Link href="/search" className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[.035] px-4 py-2.5 text-sm font-bold text-white"><Search className="size-4"/>Search</Link><Link href="/explore" className="rounded-full bg-violet-500 px-4 py-2.5 text-sm font-bold text-white">Explore</Link></div>
      </div>
      {topics.length > 0 && <div className="mt-5 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]"><Link href="/" className="shrink-0 rounded-full bg-white px-4 py-2 text-xs font-black text-slate-950">For You</Link><Link href="/clips" className="shrink-0 rounded-full border border-white/8 bg-white/[.025] px-4 py-2 text-xs font-bold text-slate-300">Clips</Link><Link href="/subscriptions" className="shrink-0 rounded-full border border-white/8 bg-white/[.025] px-4 py-2 text-xs font-bold text-slate-300">Following</Link><Link href="/trending" className="shrink-0 rounded-full border border-white/8 bg-white/[.025] px-4 py-2 text-xs font-bold text-slate-300">Trending</Link><Link href="/history" className="shrink-0 rounded-full border border-white/8 bg-white/[.025] px-4 py-2 text-xs font-bold text-slate-300">History</Link></div><div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">{topics.map(([topic, count]) => <Link key={topic} href={`/search?q=${encodeURIComponent(topic)}`} className="shrink-0 rounded-full border border-white/8 bg-white/[.025] px-3 py-1.5 text-xs font-semibold text-slate-400 hover:border-violet-300/30 hover:text-white">#{topic}<span className="ml-1.5 text-slate-600">{count}</span></Link>)}</div>}
    </section>
    ...
    {loading ? <div className="grid min-h-[42vh] place-items-center"><div className="flex items-center gap-3 text-sm text-slate-400"><Loader2 className="size-5 animate-spin text-violet-300"/>Loading your feed…</div></div> : error ? <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-white/[.03] p-8 text-center"><p className="font-semibold text-white">We couldn’t load your feed.</p><p className="mt-2 text-sm text-slate-500">{error}</p><Button onClick={() => void load()} className="mt-5 bg-violet-500 hover:bg-violet-400"><RefreshCw className="mr-2 size-4"/>Retry</Button></div> : videos.length ? <div className="space-y-11">
      ...
      <Section title="Quick access" eyebrow="Your HkTube shortcuts"><div className="grid gap-3 sm:grid-cols-3"><Link href="/clips" className="rounded-2xl border border-fuchsia-300/10 bg-fuchsia-500/[.04] p-4"><div className="flex items-center gap-2 text-fuchsia-200"><Compass className="size-4"/><span className="font-bold">Clips Feed</span></div><p className="mt-1 text-xs text-slate-500">Swipe, save and follow creators.</p></Link><Link href="/history" className="rounded-2xl border border-white/8 bg-white/[.03] p-4"><div className="flex items-center gap-2 text-violet-200"><Clock3 className="size-4"/><span className="font-bold">Watch history</span></div><p className="mt-1 text-xs text-slate-500">Continue videos you started.</p></Link><Link href="/notifications" className="rounded-2xl border border-white/8 bg-white/[.03] p-4"><div className="flex items-center gap-2 text-cyan-200"><Bell className="size-4"/><span className="font-bold">Notifications</span></div><p className="mt-1 text-xs text-slate-500">Creator and account activity.</p></Link></div></Section><Section title="Discover more" eyebrow="Other HkTube surfaces"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Link href="/posts" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 hover:border-cyan-300/30"><p className="font-bold text-white">Posts</p><p className="mt-1 text-xs text-slate-500">Community updates</p></Link><Link href="/stories" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 hover:border-amber-300/30"><p className="font-bold text-white">Stories</p><p className="mt-1 text-xs text-slate-500">24-hour updates</p></Link><Link href="/subscriptions" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 hover:border-violet-300/30"><p className="font-bold text-white">Following</p><p className="mt-1 text-xs text-slate-500">Creators you follow</p></Link><Link href="/library" className="rounded-2xl border border-white/8 bg-white/[.03] p-4 hover:border-emerald-300/30"><p className="font-bold text-white">Library</p><p className="mt-1 text-xs text-slate-500">Saved and watched</p></Link></div></Section>
