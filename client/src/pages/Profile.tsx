...
export default function Profile() {
  ...
  async function loadProfile() {
    ...
    setDataLoading(true);
    try {
      const [channels, ownVideos] = await Promise.all([
        listMySupabaseChannels(),
        listMySupabaseVideos(user.openId?.replace(/^supabase:/, "") || ""),
      ]);
  ...
  async function saveProfile(event: React.FormEvent) {
    ...
    try {
      setSaving(true);
      const updated = await updateSupabaseChannel(channel.id, {
      ...
      toast.error(error instanceof Error ? error.message : "Could not update channel profile.");
    } finally {
      setSaving(false);
    }
  }
  ...
  function openMenuLink(href: string) {
    setMenuOpen(false);
    navigate(href);
  }
          ...
          <div className="-mt-10 flex flex-col gap-4 sm:-mt-12 sm:flex-row sm:items-end">
            ...
            <div className="relative">
              ...
              {menuOpen && <div className="absolute right-0 top-12 z-50 w-64 overflow-hidden rounded-2xl border border-white/10 bg-[#080b12] p-2 text-white shadow-2xl shadow-black/50">
                ...
                <ProfileMenuItem icon={Megaphone} title="Ads & privacy" description="Advertising and consent settings" onClick={() => openMenuLink("/settings/ads")} />
                <ProfileMenuItem icon={Mail} title="Contact" description="Support and platform contact" onClick={() => openMenuLink("/contact")} />
                <ProfileMenuItem icon={Trash2} title="Data & account" description="Account data and deletion" onClick={() => openMenuLink("/delete-account")} />
                <ProfileMenuItem icon={ShieldCheck} title="Privacy" description="Privacy policy and controls" onClick={() => openMenuLink("/privacy")} />
              </div>}
            </div>
          </div>
