...
import { Bell, BarChart3, Bot, CheckCircle2, ChevronRight, CircleUserRound, Clapperboard, Clock3, Download, History, Library, LogOut, MonitorPlay, Settings, ShieldCheck, Sparkles, UserRound, Wallet, Wrench } from "lucide-react";
...
export default function Menu() {
  ...
  return <HkTubeShell title="You">
    <div className="mx-auto max-w-4xl px-4 py-5 sm:px-6 sm:py-8">
      ...
      <section className="mt-8"><SectionTitle icon={Clock3} title="History & Library" /><div className="divide-y divide-zinc-100 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">{accountRows.map(row => <AccountRow key={row.title} {...row} />)}</div></section>
      <section className="mt-8" id="creator-studio"><div className="flex items-end justify-between gap-4"><SectionTitle icon={Sparkles} title="Creator Studio" /><Link href="/studio" className="text-xs font-bold text-zinc-700 hover:text-black">Open Studio</Link></div><div className="grid gap-3 sm:grid-cols-2">{creatorRows.map(row => <AccountRow key={row.title} {...row} />)}</div><div className="mt-3 rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-zinc-950" /><div><h3 className="font-bold text-zinc-950">Advanced creator features are ready for providers</h3><p className="mt-1 text-sm leading-6 text-zinc-600">Analytics use published HkTube records. Payments, payouts, AI dubbing and automated AI actions remain clearly marked until their secure providers are connected.</p></div></div></div></section>
      {user.role === "admin" && <section className="mt-8"><SectionTitle icon={ShieldCheck} title="Owner controls" /><div className="grid gap-3 sm:grid-cols-2"><AccountRow title="Admin Panel" description="Full owner control: moderation, reports, creators, verification, analytics, audit logs and platform operations" href="/admin" icon={Wrench} /><AccountRow title="Algorithm control room" description="Recommendation health and owner-only safety checks" href="/algorithm" icon={ShieldCheck} /></div><p className="mt-3 px-1 text-xs leading-5 text-zinc-500">Admin Panel is shown only when the authenticated account has the protected HkTube admin role. Server-side owner-email checks remain required for every admin action.</p></section>}
      ...
      <button type="button" onClick={() => void logout()} className="mt-8 flex w-full items-center justify-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm font-bold text-zinc-800 transition hover:border-zinc-400 hover:bg-zinc-50 hover:text-black"><LogOut className="size-4" />Sign out</button>
    </div>
  </HkTubeShell>;
}
...
function SectionTitle({ icon: Icon, title }: { icon: typeof Settings; title: string }) { return <div className="mb-3 flex items-center gap-3"><IconBadge><Icon className="size-5" /></IconBadge><h2 className="text-lg font-black text-zinc-950">{title}</h2></div>; }
function AccountRow({ title, description, href, icon: Icon }: { title: string; description: string; href: string; icon: typeof Settings }) { return <Link href={href} className="group flex items-center gap-4 bg-white p-4 transition hover:bg-zinc-50"><IconBadge><Icon className="size-5" /></IconBadge><span className="min-w-0 flex-1"><span className="block font-bold text-zinc-950">{title}</span><span className="mt-1 block text-xs leading-5 text-zinc-500">{description}</span></span><ChevronRight className="size-5 text-zinc-400 transition group-hover:translate-x-0.5 group-hover:text-black" /></Link>; }
