...
export default function UploadPage(){
 ...
 function chooseFiles(e:ChangeEvent<HTMLInputElement>){
   ...
   void videoMeta(file).then(async info=>{
     ...
     if(mode==="video"&&(info.width<=info.height))throw new Error("Long Video mode needs a landscape video.");
 ...
 return <section>
  ...
  <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_390px]">
   <section className="rounded-[2rem] border border-white/10 bg-white/[.035] p-5 sm:p-7">
    ...
    {isVideo&&<div className="mt-4 grid gap-3 sm:grid-cols-2"><Select label="Category" value={p.category} set={p.setCategory} options={["Entertainment","Gaming","Music","Education","News","Sports","Technology","Lifestyle"]}/><Select label="Language" value={p.language} set={p.setLanguage} options={["English","Urdu","Hindi","Arabic","Other"]}/></div>}
