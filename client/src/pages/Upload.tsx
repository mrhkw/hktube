...
function Choose({onPick,onDraft}:{onPick:(m:Mode)=>void;onDraft:()=>void}){
 const cards:[Mode,string,string,string,ReactNode][]=[
...
function Info({icon,title,text}:{icon:ReactNode;title:string;text:string}){return <div className="rounded-2xl border border-white/10 bg-black/10 p-4"><span className="text-violet-300">{icon}</span><b className="ml-2 text-xs text-white">{title}</b><p className="mt-2 text-[11px] leading-5 text-slate-500">{text}</p></div>}
