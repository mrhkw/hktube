...
import { ArrowLeft, CheckCircle2, FileVideo2, ImagePlus, Loader2, PauseCircle, PlayCircle, ShieldCheck, UploadCloud, X } from "lucide-react";
...
async function generateThumbnail(file: File, info: VideoInfo): Promise<File | null> {
  return new Promise(resolve => {
    ...
    video.onloadedmetadata = () => {
      video.currentTime = Math.min(Math.max(info.duration * 0.12, 0.25), 3);
    };
    video.onseeked = () => {
      window.clearTimeout(timeout);
      ...
      {step === "publish" && notice?.type === "success" ? <section className="mx-auto max-w-2xl rounded-3xl border border-emerald-300/20 bg-emerald-500/[.06] p-8 text-center">
        ...
        <div className="space-y-5">
          {step === "media" ? <section className="rounded-3xl border border-white/10 bg-white/[.03] p-5 sm:p-7">
            ...
            <div onDragOver={event => { event.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDrop={onDrop} className={`mt-6 rounded-3xl border-2 border-dashed p-7 text-center transition ${dragActive ? "border-violet-300 bg-violet-500/10" : "border-white/12 bg-black/15"}`}>
            ...
            <div className="mt-6 space-y-5">
              <label className="block"><span className="text-sm font-bold text-white">Title <b className="text-rose-300">*</b></span><div className="mt-2 flex items-center gap-2"><input value={title} onChange={event => setTitle(event.target.value)} maxLength={100} required className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none focus:border-violet-300/50" placeholder={isShort ? "Write a strong Clip title" : "Give your video a clear title"} /><span className="text-[11px] text-slate-600">{title.length}/100</span></div></label>
