...
async function syncSupabaseIdentity(identity: { subject: string; email: string | null; metadata?: Record<string, unknown> }): Promise<User | null> {
  const displayName = typeof identity.metadata?.display_name === "string" ? identity.metadata.display_name : typeof identity.metadata?.full_name === "string" ? identity.metadata.full_name : identity.email?.split("@")[0] ?? "HkTube creator";
  ...
  if (ENV.supabaseAnonKey) {
    try {
      ...
      if (response.ok) {
        ...
        if (data.id) return syncSupabaseIdentity({ subject: data.id, email: data.email ?? null, metadata: data.user_metadata });
      }
    } catch { /* use JWT fallback */ }
  }
  try {
    const { payload } = await jwtVerify(token, supabaseJwks, { algorithms: ["ES256", "RS256"] });
...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) { console.log("[OAuth] Initialized with baseURL:", ENV.oAuthServerUrl); }
  private decodeState(state: string): string { return decodeOAuthState(state).redirectUri; }
  async getTokenByCode(code: string, state: string): Promise<ExchangeTokenResponse> { const payload: ExchangeTokenRequest = { clientId: ENV.appId, grantType: "authorization_code", code, redirectUri: this.decodeState(state) }; const { data } = await this.client.post<ExchangeTokenResponse>(EXCHANGE_TOKEN_PATH, payload); return data; }
  async getUserInfoByToken(token: ExchangeTokenResponse): Promise<GetUserInfoResponse> { const { data } = await this.client.post<GetUserInfoResponse>(GET_USER_INFO_PATH, { accessToken: token.accessToken }); return data; }
}
...
class SDKServer {
  ...
  private deriveLoginMethod(platforms: unknown, fallback: string | null | undefined): string | null { if (fallback && fallback.length > 0) return fallback; if (!Array.isArray(platforms) || platforms.length === 0) return null; const set = new Set<string>(platforms.filter((p): p is string => typeof p === "string")); if (set.has("REGISTERED_PLATFORM_EMAIL")) return "email"; if (set.has("REGISTERED_PLATFORM_GOOGLE")) return "google"; if (set.has("REGISTERED_PLATFORM_APPLE")) return "apple"; if (set.has("REGISTERED_PLATFORM_MICROSOFT") || set.has("REGISTERED_PLATFORM_AZURE")) return "microsoft"; if (set.has("REGISTERED_PLATFORM_GITHUB")) return "github"; const first = Array.from(set)[0]; return first ? first.toLowerCase() : null; }
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (sessionToken) { const supabaseUser = await authenticateSupabaseToken(sessionToken); if (supabaseUser) return supabaseUser; }
    const session = await this.verifySession(sessionToken);
    if (!session) throw ForbiddenError("Invalid session cookie");
    if (session.openId.startsWith(CRON_OPEN_ID_PREFIX)) { const userInfo = await this.getUserInfoWithJwt(sessionToken ?? ""); const taskUid = userInfo.taskUid ?? null; if (!taskUid) throw ForbiddenError("Cron session missing task_uid"); return buildCronUser(userInfo); }
    const signedInAt = new Date(); let user = await db.getUserByOpenId(session.openId);
    if (!user) { try { const userInfo = await this.getUserInfoWithJwt(sessionToken ?? ""); await db.upsertUser({ openId: userInfo.openId, name: userInfo.name || null, email: userInfo.email ?? null, loginMethod: userInfo.loginMethod ?? userInfo.platform ?? null, lastSignedIn: signedInAt }); user = await db.getUserByOpenId(userInfo.openId); } catch { throw ForbiddenError("Failed to sync user info"); } }
...
function buildCronUser(userInfo: GetUserInfoWithJwtResponse): AuthenticatedUser { const now = new Date(); return { id: -1, openId: userInfo.openId, name: userInfo.name || "Manus Scheduled Task", email: null, loginMethod: null, role: "user", createdAt: now, updatedAt: now, lastSignedIn: now } as AuthenticatedUser; }
